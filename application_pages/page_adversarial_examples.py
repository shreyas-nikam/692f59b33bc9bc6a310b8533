...
def main():
    ...
    **Risk Formula:**
    ...
    baseline_accuracy_adv = evaluate_model_performance(baseline_model_adv, test_loader)
    ...
    *Annotation:* This metric represents the percentage of correctly classified images by the model.
    ...
    epsilon = st.slider("Select Epsilon (Attack Strength)", 0.0, 0.5, 0.1, 0.01)
    ...
    if st.button("Generate Adversarial Example", key="run_ae_scenario"):
        with st.spinner("Generating adversarial example..."):
            ...
            with torch.no_grad():
                original_output = baseline_model_adv(torch.tensor(original_image, dtype=torch.float32).unsqueeze(0).unsqueeze(0))
                _, original_prediction = torch.max(original_output.data, 1)
                adv_output = baseline_model_adv(adversarial_image_tensor.unsqueeze(0).unsqueeze(0))
                _, adv_prediction = torch.max(adv_output.data, 1)

            st.subheader("Original vs. Adversarial Image")
            col1, col2 = st.columns(2)
            with col1:
                st.image(original_image, caption=f"Original (True: {class_names[original_label]}, Pred: {class_names[original_prediction.item()]})", width=200)
            with col2:
                st.image(adversarial_image_np, caption=f"Adversarial (True: {class_names[original_label]}, Pred: {class_names[adv_prediction.item()]})", width=200)
            ...
            num_adv_samples_eval = min(50, len(X_test) // 2) # Ensure we don't exceed test set size
            if num_adv_samples_eval > 0:
                for i in np.random.choice(len(X_test), num_adv_samples_eval, replace=False):
                    ...
                    adv_img = generate_adversarial_example(baseline_model_adv, torch.tensor(current_image, dtype=torch.float32), current_label, epsilon=epsilon)
                    adv_X_test_subset.append(adv_img.cpu().numpy())
                    adv_y_test_subset.append(current_label)
                
                adv_X_test_subset_tensor = torch.tensor(np.array(adv_X_test_subset), dtype=torch.float32)
                adv_y_test_subset_tensor = torch.tensor(np.array(adv_y_test_subset), dtype=torch.long)
                adv_test_dataset_subset = TensorDataset(adv_X_test_subset_tensor, adv_y_test_subset_tensor)
                adv_test_loader_subset = DataLoader(adv_test_dataset_subset, batch_size=32, shuffle=False)

                adversarial_accuracy = evaluate_model_performance(baseline_model_adv, adv_test_loader_subset)
            else:
            ...
            st.session_state["ae_adv_X_train_for_mitigation"] = adv_X_test_subset # Use these for training
            st.session_state["ae_adv_y_train_for_mitigation"] = adv_y_test_subset
            st.session_state["ae_original_train_loader"] = train_loader # Store original loader
            st.session_state["ae_test_loader"] = test_loader # Store test loader

            st.subheader("Results After Adversarial Attack")
            st.markdown(f"**Accuracy on Adversarial Examples:** `{adversarial_accuracy:.2f}%`")
            ...
            *Annotation:* **Performance Impact (%)** measures the percentage decrease in model accuracy
            ...
            """)
            st.markdown(f"**Performance Impact:** `{performance_impact_ae:.2f}%`")
            ...
            post_attack_risk_score_ae = calculate_risk_score(post_attack_p_event_ae, post_attack_m_consequence_ae)

            st.markdown(display_risk_status(post_attack_risk_score_ae), unsafe_allow_html=True)

            metrics_ae = {
                "Baseline Accuracy": baseline_accuracy_adv,
                "Adversarial Accuracy": adversarial_accuracy
            }
            st.pyplot(create_performance_chart(metrics_ae, "Model Accuracy: Baseline vs. Adversarial"))
            ...
            st.session_state["ae_baseline_model_for_mitigation"] = baseline_model_adv # Store for mitigation
            st.session_state["ae_baseline_accuracy_for_mitigation"] = baseline_accuracy_adv

            st.success("Adversarial example generation and evaluation complete!")
    ...
    if "ae_adversarial_accuracy" in st.session_state:
        ...
        if st.button("Apply Adversarial Training", key="apply_ae_mitigation"):
            with st.spinner("Applying adversarial training..."):
                ...
                mitigated_accuracy_on_original_test = evaluate_model_performance(mitigated_model_ae, st.session_state["ae_test_loader"])
                
                # Re-generate adversarial examples for the mitigated model for a fair comparison
                adv_X_test_mitigated = []
                adv_y_test_mitigated = []
                num_adv_samples_eval = min(50, len(X_test) // 2) # Use the same number of samples as before
                if num_adv_samples_eval > 0:
                    for i in np.random.choice(len(X_test), num_adv_samples_eval, replace=False):
                        current_image = X_test[i]
                        current_label = y_test[i]
                        adv_img = generate_adversarial_example(mitigated_model_ae, torch.tensor(current_image, dtype=torch.float32), current_label, epsilon=epsilon)
                        adv_X_test_mitigated.append(adv_img.cpu().numpy())
                        adv_y_test_mitigated.append(current_label)
                
                    adv_X_test_mitigated_tensor = torch.tensor(np.array(adv_X_test_mitigated), dtype=torch.float32)
                    adv_y_test_mitigated_tensor = torch.tensor(np.array(adv_y_test_mitigated), dtype=torch.long)
                    adv_test_dataset_mitigated = TensorDataset(adv_X_test_mitigated_tensor, adv_y_test_mitigated_tensor)
                    adv_test_loader_mitigated = DataLoader(adv_test_dataset_mitigated, batch_size=32, shuffle=False)
                    mitigated_accuracy_on_adversarial = evaluate_model_performance(mitigated_model_ae, adv_test_loader_mitigated)
                else:
                ...
                st.markdown(f"**Mitigated Model Accuracy (on adversarial examples):** `{mitigated_accuracy_on_adversarial:.2f}%`")

                performance_recovery_ae = mitigated_accuracy_on_adversarial - st.session_state["ae_adversarial_accuracy"]
                st.markdown(r"""
                *Annotation:* **Performance Recovery (%)** indicates the improvement in model accuracy
                ...
                """)
                st.markdown(f"**Performance Recovery:** `{performance_recovery_ae:.2f}%`")

                mitigated_risk_score_ae = calculate_risk_score(mitigated_p_event_ae, mitigated_m_consequence_ae)
                st.markdown(display_risk_status(mitigated_risk_score_ae), unsafe_allow_html=True)
                ...
                st.pyplot(create_performance_chart(metrics_mitigated_ae, "Model Accuracy: Baseline, Adversarial, Mitigated"))
                ...
                st.success("Adversarial training mitigation complete and risk registered!")
